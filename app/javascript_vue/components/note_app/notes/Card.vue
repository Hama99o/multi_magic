<template>
  <v-infinite-scroll
    v-if="notes?.length"
    class="overflow-hidden px-2 md:!px-0"
    :items="notes"
    :onLoad="load"
  >
    <div
      :class="
        isMobile || noteIndexType != 'card_grid'
          ? 'list-masonry'
          : 'masonry sm:masonry-sm md:masonry-md'
      "
    >
      <v-card
        ...
        class="d-flex flex-column just mx-auto mb-[0.7em]"
      >
        <div
          :class="openNoteId == item.id ? 'invisible overflow-hidden' : ''"
          class="m-3 flex flex-col gap-1"
        >
          <div
            class="truncate bg-background p-2 font-semibold"
            v-html="highlight(item.title || 'Untitled Note')"
          ></div>
          <span
            class="note-container line-clamp-22 line-clamp-[22] max-h-[500px] whitespace-normal"
            v-html="highlight(item.description)"
          />
        </div>

        <div v-if="openNoteId != item.id" class="flex flex-col gap-2">
          <div v-if="item.tags?.length" class="mt-5 flex flex-wrap">
            <div
              v-for="tag in item.tags.filter((x) => item.tag_ids.includes(x.id)).slice(0, 4)"
              :key="tag.id"
            >
              <v-chip
                class="ma-1"
          ...
          <div v-if="item?.shared_users?.length" class="ml-1 flex flex-wrap">
        ...
        <div v-if="isTrash" class="mx-1 flex justify-between">
          <v-btn icon size="small" @click.stop="trashNoteDeletePermanently(item)">
            <v-icon class="text-primary" size="32" :icon="'mdi mdi-delete'" />
          </v-btn>

          <v-btn icon size="small" @click.stop="trashNoteRestore(item)">
  ...
  <div v-if="!notes?.length" class="flex items-center justify-center pt-[300px]">
    <span class="my-12">No more notes!</span>
  </div>
</template>
...
const trashNoteRestore = async (trashesItem) => {
  ...
  showToast(
    `${trashesItem.title || trashesItem.item || trashesItem.contact_name} ${
      trashesItem.name
  ...
  try {
    openPopUp({
      ...
      async confirm() {
        ...
        showToast(
          `${
            trashesItem.title || trashesItem.item || trashesItem.contact_name || 'Note '
